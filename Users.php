...
class BaseUser
{
	...
	public function GetAccountBalanceFromServer($PubKey){
		...
		socket_close($socket);
		$raw_data = file_get_contents('http://35.240.207.160:3000/getBalance');
	...
	public function createEthereumAccount(){
		...
		socket_close($socket);
		$raw_data = file_get_contents('http://35.240.207.160:3000/GetNewAccount');
		$data = json_decode($raw_data, true);
...
class StandardUser extends BaseUser 
{
	...
	public function __construct($Object){
		...
		public function checkAccountInNetwork($WalletPubKey){
			...
			socket_close($socket);
			$raw_data = file_get_contents('http://35.240.207.160:3000/CheckAccount');
			$data = json_decode($raw_data, true);
			if ($data['RESPONSE']){
				return true;
		...
		socket_close($socket);
		$raw_data = file_get_contents('http://35.240.207.160:3000/ConvertToSTICoin');
		$data = json_decode($raw_data, true);
		...
		socket_close($socket);
		$raw_data = file_get_contents('http://35.240.207.160:3000/ConvertToETH');
		$data = json_decode($raw_data, true);
		if($data['Transaction']== "Success"){
			$this->AccountBalance  = $data['sticoin'];
			return $data['Transaction'];
