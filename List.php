...
$NameErr = $ManufactureYrErr = $CategoryErr = $CaptionErr = $DescriptionErr = $CostErr = $Cost_OverErr = $FileErr= "";
...
if (empty($_POST["ManufactureYr"])){
	$ManufactureYrErr= "ManufactureYr is required";	
	$submit = false;
}
else{
	$ManufactureYr = test_input($_POST["ManufactureYr"]);
		
		if(substr($ManufactureYr, -2) < 0 || substr($ManufactureYr, -2) > substr( date("Y"), -2)||strlen($ManufactureYr)<4||$ManufactureYr<1000){
			$submit = false;
			$ManufactureYrErr = "Invalid Manufacture Year";
		}
}
